#!/usr/bin/env node
/** Fail if the board shell is missing, fat, or not v21. Data syncs must never rewrite these files. */
...
async function check() {
  ...
  if (!html.includes("Board v21")) problems.push("index.html missing 'Board v21'");
  ...
  if (!board.includes('BOARD_VERSION = "v21"')) problems.push("board.js is not v21");
  if (problems.length) {
    console.error("UI LOCK FAIL:\n - " + problems.join("\n - "));
    console.error("See BOARD_UI_LOCK.md. Restore index.html + board.js from a v21 commit. Never write them from a data sync.");
    process.exit(1);
  }
  console.log(`UI LOCK ok — index ${size} bytes, Board v21, board.js intact`);
}
