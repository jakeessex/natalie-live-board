...
|---|---|---|
| `index.html` | Thin Board v19 shell (~9KB). Gate label `Board v19`. | Humans restoring the UI only |
| `board.js` | v19 sales pipeline | Humans restoring the UI only |
...
- `index.html` does not say `Board v19`
- `index.html` is over 40KB (fat embed) or under 2KB (empty push)
- `board.js` is not `BOARD_VERSION = "v19"`
